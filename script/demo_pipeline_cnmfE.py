...
def main():
    ...
    # Set neither by CLI arg nor through JSON, so use default data
    # if not opts.data['fnames']: 
    #     fnames = [caiman.utils.utils.download_demo('data_endoscope.tif')]
    ...
    c, dview, n_processes = caiman.cluster.setup_cluster(
        backend=cfg.cluster_backend, n_processes=cfg.cluster_nproc
    )
    # Motion Correction
    ...
    if not cfg.no_motion_correct:
        ...
        fname_new = caiman.save_memmap(
            opts.data['fnames'], base_name='memmap_', 
    ...
    opts.change_params(params_dict={
        'dims': dims,           # we rework the source files
    ...
    caiman.utils.visualization.inspect_correlation_pnr(cn_filter, pnr)
    ...
    if not cfg.no_play:
        ...
        cnm.estimates.plot_contours(
            img=cn_filter, idx=cnm.estimates.idx_components
        )
        cnm.estimates.view_components(
            images, idx=cnm.estimates.idx_components
        )
        # fully reconstructed movie
        cnm.estimates.play_movie(
            images, q_max=99.5, magnification=2,
            include_bck=True, gain_res=10, bpx=bord_px
        )
        # movie without background
        cnm.estimates.play_movie(
            images, q_max=99.9, magnification=2, 
...
def handle_args():
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            caiman.paths.caiman_datadir(), 
            'demos', 'general', 'params_demo_pipeline_cnmfE.json'
        ), 
        help="JSON Configfile for Caiman parameters"
    )
    parser.add_argument(
        "--no_motion_correct", action='store_true', 
        help="Set to disable motion correction"
    )
    parser.add_argument(
        "--keep_logs",  action="store_true", 
        help="Keep temporary logfiles"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--cluster_backend", default="multiprocessing", 
        help="Specify multiprocessing, ipyparallel, or single to pick an engine"
    )
    parser.add_argument(
        "--cluster_nproc", type=int, default=None, 
        help="Override automatic selection of number of workers to use"
    )
    parser.add_argument(
        "--input", action="append", 
        help="File(s) to work on, provide multiple times for more files"
    )
    parser.add_argument(
        "--logfile",
        help="If specified, log to the named file"
    )
