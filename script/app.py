...
def _resolve_app_paths(cfg: omegaconf.DictConfig) -> tuple[Path, Path, Path, Path]:
    ...
    crop_load_fold = _resolve_optional_path(cfg.crop_load_fold)
    ...
    if data_fold is not None:
        ...
        crop_load_fold = crop_load_fold or (data_fold / "crop")
    ...
    if missing:
        raise ValueError(
            ...
            "Provide data_fold, or set all of bg_load_path, extract_load_fold, crop_load_fold, and app_fold. "
            f"Missing: {', '.join(missing)}"
        )

    return bg_load_path, extract_load_fold, crop_load_fold, app_fold
...
def main(cfg: omegaconf.DictConfig) -> None:
    bg_load_path, extract_load_fold, crop_load_fold, app_fold = _resolve_app_paths(cfg)
    run_app(
        bg_load_path=bg_load_path,
        extract_load_fold=extract_load_fold,
        crop_load_fold=crop_load_fold,
