...
def run(
    ...
    same_path = (
        os.path.abspath(x_load_path) == os.path.abspath(x_save_path)
    )
    if not same_path:
        tmp_path = x_save_path
    else:
        tmp_path = x_save_path + ".tmp_norm.tif"
    ...
    if same_path:
        os.replace(tmp_path, x_save_path)
